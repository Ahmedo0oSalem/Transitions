...
def _df_to_table_figure(df, title, metric_col='mean_home_control'):
    ...
    Supports both:
      - long format: one row per (window, team), single metric column (e.g. 'mean_obso')
      - wide format: one row per window, separate 'mean_home_control'/'mean_away_control' columns
    """
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    if is_wide:
        ...
        # Aggregate in case of any residual duplicate keys (defensive)
        home_vals = df.groupby(key_cols)['mean_home_control'].mean()
        away_vals = df.groupby(key_cols)['mean_away_control'].mean()

        home_series = [
            home_vals.get((r.period, r.windowStartSec, r.windowEndSec), np.nan)
            for r in windows.itertuples()
        ]
        away_series = [
            away_vals.get((r.period, r.windowStartSec, r.windowEndSec), np.nan)
            for r in windows.itertuples()
        ]
        ...
        home_series = [
            home_map.get((r.period, r.windowStartSec, r.windowEndSec), np.nan)
            for r in windows.itertuples()
        ]
        away_series = [
            away_map.get((r.period, r.windowStartSec, r.windowEndSec), np.nan)
            for r in windows.itertuples()
        ]
...
def obso_figure(match_id, processed_dir, epv_grid_path=None, radius=5.0):
    ...
    if not window_df.empty:
        print("\n--- OBSO DEBUG ---")
        print(window_df['team'].value_counts(dropna=False))
        print(window_df[['windowStartSec', 'windowEndSec', 'team', 'mean_obso']].head(10))
        print("------------------")
