...
def build_formation_segments(match_id: str | int, processed_dir: str) -> pd.DataFrame:
    ...
    logger.info("[%s] loaded %s possession sequences for segment aggregation.", match_id, len(possession_seqs))
        ...
        for _, row in grp.iterrows():
            ...
            if formation == current_formation:
                ...
                if current_formation is not None:
                    seg_data = _finalize_segment(
                        match_id, team, period, current_formation, 
                        seg_start, seg_end, df, possession_seqs
                    )
...
def _finalize_segment(match_id, team, period, formation, start, end, df, possession_seqs):
    ...
    first_row = segment_windows.iloc[0]
