...
Two signal types are combined multiplicatively:
...
def _classify_events(events):
    ...
    # Group consecutive OTB events: the span from the first OTB to the
    # last consecutive OTB + *SETPIECE_RECOVERY_SECONDS* is unreliable.
    i = 0
    while i < len(otb_events):
        ...
        if period is None or span_start is None:
            i += 1
            continue
        # Consume consecutive OTB events in the same period
        j = i + 1
        while j < len(otb_events):
            if otb_events[j].get("period") != period:
                break
            next_sec = otb_events[j].get("periodElapsedTimeEstimate")
            if next_sec is None:
                j += 1
                continue
            j += 1
        span_end = otb_events[j - 1].get("periodElapsedTimeEstimate", span_start)
...
def _detect_turnovers(tracking_path, pitch_length, pitch_width):
    """Simple proximity-based ball-owner detection yielding turnover times.

    Returns a list of ``(period, elapsed_sec)`` for each frame where
    the ball owner switches between teams.
    """
                ...
                for team in _TEAM_KEYS:
                    for p in frame.get(team, []):
                        ...
                        if parsed is None:
                            continue
                        _pid, px, py = parsed
                        d = math.hypot(px - bx_f, py - by_f)
                        if d < best_dist:
                            best_dist = d
                            best_team = team

                if prev_owner is not None and best_team is not None and best_team != prev_owner:
                    turnovers.append((period, elapsed))

                if best_team is not None:
                    prev_owner = best_team
...
def compute_frame_weights(tracking_path, metadata, events=None, goalkeepers=None):
    ...
    for i in range(n):
        ...
        # Apply event disruptions — linear ramp from base_weight at event
        # time back to 1.0 at the end of the decay window.
        ...
        # Apply turnover disruption (both teams, short decay)
